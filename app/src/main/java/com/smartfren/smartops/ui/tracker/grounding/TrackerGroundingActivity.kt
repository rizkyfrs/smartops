package com.smartfren.smartops.ui.tracker.grounding
...
@AndroidEntryPoint
class TrackerGroundingActivity : BaseActivity() {
    ...
    private fun setupUI() {
        tvTitleAppBar.text = getString(R.string.grounding)
        clTwo.hide()
    ...
    private fun setupRVOne() {
        tvTitleOne.text = getString(R.string.tracker_issue_status)
    ...
    private fun getTrackerGrounding() {
        trackerViewModel.trackerGroundingDataList()
        trackerViewModel.trackerGroundingData.observe(this) { event ->
                    ...
                    is Resource.Success -> {
                        ...
                        response.data?.let { trackerResponse ->
                            if (trackerResponse.success == true) {
                                ...
                                for (data in trackerResponse.vSiteGroundingTracker!!) {
                                    menuList.add(
                                        ItemTable(
                                            data?.regionNw,
                                            data?.total,
