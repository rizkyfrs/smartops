package com.smartfren.smartops.ui.tracker.imb
...
@AndroidEntryPoint
class TrackerIMBActivity : BaseActivity() {
    ...
    private fun setupUI() {
        tvTitleAppBar.text = getString(R.string.imb)
        clTwo.hide()
    ...
    private fun setupRVOne() {
        tvTitleOne.text = getString(R.string.imb_issue_status)
    ...
    private fun getTrackerIMB() {
        trackerViewModel.trackerIMBDataList()
        trackerViewModel.trackerIMBData.observe(this) { event ->
                    ...
                    is Resource.Success -> {
                        ...
                        response.data?.let { trackerResponse ->
                            if (trackerResponse.success == true) {
                                ...
                                for (data in trackerResponse.vSiteImbTracker!!) {
                                    menuList.add(
                                        ItemTable(
                                            data?.regionNw,
                                            data?.total,
