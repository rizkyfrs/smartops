package com.smartfren.smartops.ui.tracker.ac
...
@AndroidEntryPoint
class TrackerACActivity : BaseActivity() {
    ...
    private fun setupUI() {
        tvTitleAppBar.text = getString(R.string.air_cond)
        clTwo.hide()
    ...
    private fun setupRVOne() {
        tvTitleOne.text = getString(R.string.air_cond_issue_status)
    ...
    private fun getTrackerAC() {
        trackerViewModel.trackerACDataList()
        trackerViewModel.trackerACData.observe(this) { event ->
                    ...
                    is Resource.Success -> {
                        ...
                        response.data?.let { trackerResponse ->
                            if (trackerResponse.success == true) {
                                ...
                                for (data in trackerResponse.cvSiteAirCondTracker!!) {
                                    menuList.add(
                                        ItemTable(
                                            data?.regionName,
                                            data?.total,
