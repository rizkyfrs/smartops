package com.smartfren.smartops.ui.tracker.worstcell
...
@AndroidEntryPoint
class TrackerWortCellActivity : BaseActivity() {
    ...
    private fun setupUI() {
        tvTitleAppBar.text = getString(R.string.worstcell)
        clTwo.hide()
    ...
    private fun setupRVOne() {
        tvTitleOne.text = getString(R.string.worst_cell_repetitive_issue_status)
    ...
    private fun getTrackerWorstCell() {
        trackerViewModel.trackerWorstCellDataList()
        trackerViewModel.trackerWorstCellData.observe(this) { event ->
                    ...
                    is Resource.Success -> {
                        ...
                        response.data?.let { trackerResponse ->
                            if (trackerResponse.success == true) {
                                ...
                                for (data in trackerResponse.vSiteWorstcellTracker!!) {
                                    menuList.add(
                                        ItemTable(
                                            data.regionNw,
                                            data.total,
